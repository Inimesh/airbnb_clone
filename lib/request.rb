...
class Request
  ...
  def self.add_request(availability_id:, user_id:)
    ...
    Request.new(request_id: rs[0]['request_id'], availability_id: rs[0]['availability_id'], 
booked: rs[0]['booked'], user_id: rs[0]['user_id'])
  end
  ...
  def self.duplicate?(availability_id, user_id)
    rs = DatabaseConnection.query(
"SELECT EXISTS(SELECT * FROM request WHERE availability_id = $1 AND user_id = $2)", [
  availability_id, user_id])
